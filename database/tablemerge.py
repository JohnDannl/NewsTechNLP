...
def getTitleSummary(tablename,limit=5000):
    query = "SELECT id,title,summary,ctime,source FROM " + tablename +' order by title desc,ctime desc limit %s'
    rows = dbconn.Select(query, (limit,))
    return rows

def getTitles(tablename,limit=5000):
    query = "SELECT id,title,summary,ctime,source FROM " + tablename +' order by id desc limit %s'
    rows = dbconn.Select(query, (limit,))
    return rows
