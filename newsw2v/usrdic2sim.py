...
def getSimNewsRep(rep_file,news_file,sim_news_file,useusrDict=False):
    corpus=list(open(news_file,'r'))
    corpus_len=len(corpus)
    ...
    for i in xrange(rep_len):           
        ...
        for j in xrange(corpus_len):            
            wl2=corpus[j].split()[1:]
            sim=w2vword2sim.getSimofWordListPairMod(wl1, wl2, modify=False,usrDict=useusrDict)
...
def getSimNewsId():
    sim_news_file='/home/dannl/tmp/newstech/usrdic/0/sim_news.txt'
    sim_mtid_file='/home/dannl/tmp/newstech/usrdic/sim_mtid'
    sim_news=list(open(sim_news_file,'r'))
    fout=open(sim_mtid_file,'w')
    count=0
    for line in sim_news:        
        count+=1                
        if count==1:
            _mtid=line.split()[0]
        else:
...
# getSimNewsRep(news_rep_file,news_file,sim_news_file,useusrDict=False)
